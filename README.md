...
```sh
set UNITELABS_CDK_APP=unitelabs.pi_4_weather_station:create_app
```

Setting this environment variable will allow you to run varous CLI commands without providing `--app unitelabs.pi_4_weather_station:create_app` every time.
...
```sh
python -m pip install unitelabs-pi-4-weather-station \
...
- On **Windows**:
  ...
  ```sh
  config show --app unitelabs.pi_4_weather_station:create_app
  ```
...
- On **Windows**:
  ...
  ```sh
  config create --app unitelabs.pi_4_weather_station:create_app
  ```
...
- On **Windows**:
  ...
  ```sh
  config create --app unitelabs.pi_4_weather_station:create_app  --path <path to config>
  ```
...
```sh
connector start --app unitelabs.pi_4_weather_station:create_app -vvv
```
...
```sh
connector start --app unitelabs.pi_4_weather_station:create_app -cfg <path to config> -vvv
```
...
```sh
git clone https://github.com/rwaelder/pi-4-weather-station.git
```
...
```sh
uv run connector dev --app unitelabs.pi_4_weather_station:create_app
```
...
[issue-tracker]: https://github.com/rwaelder/pi-4-weather-station/issues
